...
class BaseAgent:
    ...
    #         print(f"[ERROR] Failed loading table {filename}: {e}")
    #         return pd.DataFrame()
    def _load_table(self, filename: str) -> pd.DataFrame:
        path = os.path.join(self.ruleset_folder, filename)
        try:
            ...
            if df.empty or df.shape[1] == 0:
                raise ValueError("Empty or malformed HTML table")
            return df
        except Exception as e:
            print(f"[ERROR] Failed loading table {filename}: {e}")
            return pd.DataFrame()
...
def setup_ruleset_tools(mcp: FastMCP):
    ...
    def evaluate_ruleset(query: str) -> Dict[str, str]:
        ...
        # Directly use the input string (formatted markdown-like key-value block)
        cleaned_query = clean_markdown_to_kv(query)
        results = config_system.process_query(cleaned_query, verbose=True)
        # results = config_system.process_query(query, verbose=True)
        print("hello world!!!!")
        # Print report
