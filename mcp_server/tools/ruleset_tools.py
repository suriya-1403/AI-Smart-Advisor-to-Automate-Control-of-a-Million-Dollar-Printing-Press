...
def extract_inputs_from_natural_query(query, model_name="llama3.2"):
    prompt = PromptTemplate(
...
def setup_ruleset_tools(mcp: FastMCP):
    ...
    def evaluate_ruleset(query: str) -> str:
        ...
        config_system = ConfigurationSystem(
            ruleset_folder="/Users/suriya/Documents/Github/AI-Smart-Advisor-to-Automate-Control-of-a-Million-Dollar-Printing-Press/mcp_server/data/rulesets"
        )
        ...
        formatted_query = "\n".join(
            [f"{k} - {v}" for k, v in structured_inputs.items()]
        )
